...
thumb=thumb.replace(/^https:\/\//i,'https://');y='<a href="'+q+'"><img src="'+thumb+'" alt="" /></a>';}
...
function linkify(text,type){text=text.replace(/((https?\:\/\/)|(www\.))(\S+)(\w{2,4})(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/gi,function(url){var full_url=!url.match('^https?:\/\/')?'https://'+url:url;return'<a href="'+full_url+'">'+url+'</a>';});if(type=='twitter'){text=text.replace(/(^|\s)@(\w+)/g,'$1@<a href="https://www.twitter.com/$2">$2</a>');text=text.replace(/(^|\s)#(\w+)/g,'$1#<a href="https://twitter.com/search/%23$2">$2</a>');}else if(type=='facebook'){text=text.replace(/<\/br>/g,' ');text=text.replace(/(^|\s)#(\w+)/g,'$1#<a href="https://facebook.com/hashtag/%23$2">$2</a>');}
return text;}
...
image=image.replace("_m.jpg",".jpg");if(image.indexOf('fbcdn')==-1){image=image.replace("_b.jpg","_f.jpg");image=image.replace("_b.png","_f.png");}else{if(image.indexOf('safe_image.php')==-1){var id=image.split("_"),object_id=id[1];image='https://graph.facebook.com/'+object_id+'/picture?type=normal';}}
...
if(this.prefix=='https://'){var image_tmp=image.replace('https://','https://');image=image_tmp;}}else{return false}
return image;}
...
function flickrHrefLink(id,obj){jQuery.ajax({url:'https://api.flickr.com/services/feeds/groups_pool.gne?id='+id+'&format=json&jsoncallback=?',dataType:'jsonp',success:function(a){$('.icon',obj).each(function(){$(this).parent().attr('href',a.link);});}});}
function share(st,sq,twitterId){var s='',sq=encodeURIComponent(sq),st=encodeURIComponent(st);s='<a href="https://www.facebook.com/sharer.php?u='+sq+'&t='+st+'" class="share-facebook" target="_blank"></a>';s+='<a href="https://twitter.com/share?url='+sq+'&text='+st+'&via='+twitterId+'" class="share-twitter" target="_blank"></a>';s+='<a href="https://plus.google.com/share?url='+sq+'" class="share-google" target="_blank"></a>';s+='<a href="https://www.linkedin.com/shareArticle?mini=true&url='+sq+'&title='+st+'" class="share-linkedin" target="_blank"></a>';return s;}})(jQuery);
